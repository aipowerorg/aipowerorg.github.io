...
- `[current_attributes]`: This shortcode will be replaced with a list of the product's current attributes. Please note that this shortcode is valid in both the "Short Description" and "Full Description" fields.

Here's an example of how you might use these variables in a custom prompt:

"Use the existing description: [current_short_description]. Then, provide a compelling and concise summary in English for the following product: %s. Also, include these attributes: [current_attributes]."

In this example, `[current_short_description]` will be replaced with the product's short description, and `%s` will be replaced with the product's title, and `[current_attributes]` will be replaced with the product's attributes.
