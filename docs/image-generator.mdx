...
import SD1 from '/img/image-generator/sd-models1.png';
import SD2 from '/img/image-generator/sd-models2.png';
import SD3 from '/img/image-generator/sd-models3.png';
import SD4 from '/img/image-generator/sd-models4.png';
import allmodels from '/img/image-generator/models.png';
...
Check out the demo [here](https://aipower.org/image-generator-dall-e/).
...
Our plugin supports three distinct DALL-E models provided by OpenAI, each with unique capabilities and image generation features.

<img src={allmodels} />

### DALL-E 3

DALL-E 3 is the latest iteration of OpenAI's image generation models. It allows users to create images from scratch based on a text prompt.

- **Image Sizes:** DALL-E 3 can generate images with resolutions of 1024x1024, 1024x1792, or 1792x1024 pixels.
- **Quality Settings:** By default, images are generated at standard quality. For enhanced detail, you can opt for the "hd" quality setting when using DALL-E 3.
- **Generation Speed:** Square, standard quality images are the fastest to generate.
- **Generation Quantity:** You can request 1 image at a time.

### DALL-E 3 HD

As an extension of DALL-E 3, the DALL-E 3 HD model offers images with high definition and intricate details.

- **Image Sizes:** The same as DALL-E 3, with the addition of "hd" quality for even more detailed imagery.
- **Quality and Detail:** This model will provide images with superior clarity and depth.
- **Generation Quantity:** Similar to DALL-E 3, only 1 image can be requested at a time.

### DALL-E 2

DALL-E 2 is the predecessor to DALL-E 3 but still offers a robust set of features.

- **Image Sizes:** DALL-E 2 supports 256x256, 512x512, and 1024x1024 image sizes.
- **Generation Quantity:** Up to 10 images can be generated in a single request.

:::caution
Please note that when submitting a generation request to DALL-E 3, OpenAI may automatically revise it for safety and to add more detail. Detailed prompts tend to yield higher quality images.
:::

To use these models within our plugin:

1. Ensure you have the [API key](/docs/ai-engine/openai/api-key.mdx) for DALL-E set up in the plugin settings.
2. Navigate to **Image Generator** and input your prompt.
3. Select the desired model and image specifications.
4. Hit the **Generate** button and await your unique AI-generated image.

:::info
Our plugin has been updated to support the new pricing tiers associated with each model and image size. The cost will be calculated dynamically based on your selection.
:::

:::info
Not all prompts may lead to image generation due to OpenAI's strict [content policy](https://labs.openai.com/policies/content-policy). If a prompt or image does not comply, an error message will be displayed.
:::

Remember to explore our plugin's other features, including fine-tuning settings, handling pricing, and reviewing detailed logs, to enhance your image generation experience.
...
### How to Use Different Models on Replicate

Replicate offers a plethora of AI models beyond just Stable Diffusion. With our plugin, you have the flexibility to tap into any of these models and harness their capabilities.

<iframe width="100%" height="450" src="https://www.youtube.com/embed/jDVsK16IOsE" title="Text to GIF AI Power WordPress Plugin" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>

For instance, you can generate animated GIFs from text.

Check out the demo [here](https://aipower.org/text-to-gif-test/).

Here are the steps:

1. Navigate to Replicate's website.
2. Click on the `Explore` link.

<img src={SD1} />

3. Search for your desired model, for example, [hotshot-xl](https://replicate.com/lucataco/hotshot-xl). This model allows you to generate animated GIFS from a given text.
4. Once located, go to the `Versions` tab of the model.

<img src={SD2} />

5. Copy the latest version number.

Back in our plugin:
1. Access the `Settings` and then the `Image` tab.
2. Paste the version number you copied into the `Version` field.

<img src={SD3} />

3. Save your changes.

When you're ready to generate an image:

1. Proceed to the `Image Generator` section and go to the `Stable Diffusion` tab.
2. Scroll down, and you'll see a `Scheduler` dropdown on the right.
3. Choose `Custom`. A text field will manifest.

<img src={SD4} />

4. Input the custom scheduler name. If you're in doubt about this, valid scheduler names can be found [here](https://replicate.com/lucataco/hotshot-xl/api) for this specific model.
5. Click "save as default".
6. Now, draft your prompt, press the `Generate` button, and be mesmerized as your text transforms into an animated GIF!
...
If you wish to add custom styling to your image generator form, you can follow below steps:
...
As an example, if you want to change the image generator form layout, you can add the following CSS rule:

```
/* Style the main form */
#wpaicg-image-generator-form {
...
#wpaicg-image-generator-form input,
#wpaicg-image-generator-form textarea {
  width: 100%;
  padding: 15px;
  margin: 10px 0;
...
/* Style labels */
#wpaicg-image-generator-form label {
  font-weight: bold;
}

/* Style buttons */
#wpaicg-image-generator-form button {
...
/* Button hover state */
#wpaicg-image-generator-form button:hover {
