...
For instance, you can generate animated GIFs from text. Here are the steps:
...
With our new feature, "Text to GIF", you're no longer bound to just the Stable Diffusion model when using Replicate. There's a vast universe of models out there, and we want you to experience them all. 

To switch to a different model:
1. Head over to Replicate's website.
2. Click on the `Explore` link.
3. Search for the "hotshot-xl" model.
4. Navigate to the `Versions` tab of the model.
5. Copy the latest version number.

Back in our plugin:
1. Go to `Settings` and then the `Image` tab.
2. Paste the version number you copied into the `Version` field.
3. Save your changes.

When you're ready to generate an image:
1. Head over to the `Image Generator` section and select the `Stable Diffusion` tab.
2. Scroll down and you'll find a `Scheduler` dropdown on the right.
3. Select `Custom`. A text field will appear.
4. Enter the custom scheduler name. If you're unsure what to put, you can find valid scheduler names [here](https://replicate.com/lucataco/hotshot-xl/api).
5. Click "save as default".
6. Now, just write your prompt, hit the `Generate` button, and watch as an animated GIF forms from your text!

![ImageGenerator](/img/image-generator/image-generator.jpeg)
![APIKey](/img/ai-settings/api-key.png)
![SDAPIKey](/img/image-generator/sd-api.png)
![SDAPISave](/img/image-generator/sd-api-save.png)
